...
class IrActionsServer(models.Model):
    ...
    def _safe_execute(self, action):
        """ Executes a server action inside a try except block and log results

            :param action (ir.actions.server): server action to be executed
        """
    ...
    _executing_msg = _(u'Executing the server action: {} / {}')

    _error_msg = _(u'''
        An error has occurred when Odoo was trying to execute a server action.
        Action: {} / {}
        Exception: {}
    ''')
